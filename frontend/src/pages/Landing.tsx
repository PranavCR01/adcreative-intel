...
import {
  IconArrow, IconLinkedin, IconGithub, IconUser,
} from '../components/icons'

export default function Landing({ navigate }: { navigate: (p: string) => void }) {
  return (
    <>
...
function ProblemSection() {
  const cards = [
    { idx: '01', title: 'Manual creative tagging doesn\'t scale', body: 'Performance teams hand-label every new asset by hue, theme, character — then re-label it next sprint. The taxonomy is stale before the campaign launches.', chips: [{ t: 'today', cls: 'alert' }, { t: 'human-in-the-loop', cls: '' }] },
    { idx: '02', title: 'DSPs grade your creative as a black box', body: 'Bidders give you a creative score with no provenance. You learn it\'s bad after the budget is spent. There\'s no rubric to iterate against.', chips: [{ t: 'opaque', cls: 'alert' }, { t: 'no rubric', cls: '' }] },
    { idx: '03', title: 'Nobody can tell you why', body: 'Even when fatigue is detected, the signal is a number. Was it the headline? The face? The lack of motion? Without explanation, the next creative is a guess.', chips: [{ t: 'no explanation', cls: 'alert' }, { t: 'no learning loop', cls: '' }] },
  ]
  return (
    <section className="section">
      <div className="section-inner">
        <div className="section-head">
          ...
          <p style={{ color: 'var(--text-3)', fontSize: 16, margin: 0, lineHeight: 1.6 }}>Predict fatigue before launch. Explain a score in plain language. Tell you which pixel of the creative is doing the work.</p>
        </div>
          ...
          {cards.map((c) => (
            <div className="problem-card" key={c.idx}>
              ...
              <div className="tag-row">
                {c.chips.map((ch, i) => <span key={i} className={'chip ' + ch.cls}>{ch.t}</span>)}
              </div>
            </div>
          ))}
        </div>
